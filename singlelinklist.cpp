...
SingleLinkList::SingleLinkList(Widget* widget)//构造函数
{
...
void SingleLinkList::insertAtIndex(int index, int val)//在索引index前插入元素值为val
{
...
void SingleLinkList::deleteAtIndex(int index)//删除索引index处的结点
{
...
void SingleLinkList::modifyAtIndex(int index, int val)//将index处结点的元素值修改为val
{
    if (index < 0 || index >= length)
    {
